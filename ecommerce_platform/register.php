...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (!$email || $pass !== $pass2) {
    $error = "Vérifie tes informations (email ou mots de passe).";
  } else {
    $hash = password_hash($pass, PASSWORD_DEFAULT);
    $stmt = $pdo->prepare(
      "INSERT INTO users (first_name, last_name, email, phone, job, password_hash)
             VALUES (:f, :l, :e, :p, :j, :h)"
    );
    try {
      $stmt->execute([
...
?>
<?php include 'header.php'; ?> <!-- NAVBAR APRÈS le traitement PHP -->
        ...
        <form method="post" action="register.php">
          <div class="row">
            ...
            <div
              ...
              <span
                class="px-2 small text-muted font-weight-bold text-muted">OR</span>
